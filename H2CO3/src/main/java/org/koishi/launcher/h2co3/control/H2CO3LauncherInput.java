...
import org.koishi.launcher.h2co3launcher.keycodes.AndroidKeycodeMap;
...
import org.koishi.launcher.h2co3launcher.keycodes.GamepadKeycodeMap;
...
public class H2CO3LauncherInput implements View.OnCapturedPointerListener, GamepadHandler {
    ...
    public static final HashMap<Integer, Integer> MOUSE_MAP = new HashMap<Integer, Integer>() {
        {
            put(MOUSE_LEFT, H2CO3LauncherBridge.Button1);
            put(MOUSE_MIDDLE, H2CO3LauncherBridge.Button3);
            put(MOUSE_RIGHT, H2CO3LauncherBridge.Button2);
            put(MOUSE_SCROLL_UP, H2CO3LauncherBridge.Button4);
            put(MOUSE_SCROLL_DOWN, H2CO3LauncherBridge.Button5);
        }
    };
    ...
    public H2CO3LauncherInput(@NonNull GameMenu menu) {
        this.menu = menu;
    ...
    private boolean handleMouse(MotionEvent event, float deltaTimeScale) {
        ...
        if (event != null) {
            ...
            if (gameMenu.getHitResultType() == H2CO3LauncherBridge.HIT_RESULT_TYPE_ENTITY) {
    ...
    public boolean handleKeyEvent(KeyEvent event) {
        int h2co3LauncherKeycode = AndroidKeycodeMap.convertKeycode(event.getKeyCode());
        ...
        if (h2co3LauncherKeycode == H2CO3LauncherKeycodes.KEY_UNKNOWN)
            return (event.getFlags() & KeyEvent.FLAG_FALLBACK) == KeyEvent.FLAG_FALLBACK;
        sendKeyEvent(h2co3LauncherKeycode, event.getAction() == KeyEvent.ACTION_DOWN);
        if (event.getAction() == KeyEvent.ACTION_DOWN && menu.getCursorMode() == H2CO3LauncherBridge.CursorEnabled) {
