...
import org.koishi.launcher.h2co3.control.H2CO3LauncherInput;
...
public class TouchPad extends View {
    ...
    private final Runnable runnable = () -> {
        if (!gameMenu.getMenuSetting().isDisableGesture()) {
            if (getGestureMode() == GestureMode.BUILD) {
                gameMenu.getInput().sendKeyEvent(H2CO3LauncherInput.MOUSE_LEFT, true);
                cancelMouseLeft = true;
                cancelMouseRight = false;
            } else {
                gameMenu.getInput().sendKeyEvent(H2CO3LauncherInput.MOUSE_RIGHT, true);
    ...
    private GestureMode getGestureMode() {
        if (gameMenu.getMenuSetting().isDisableBEGesture()) {
            ...
            if (gameMenu.getBridge() != null) {
                if (gameMenu.getHitResultType() == H2CO3LauncherBridge.HIT_RESULT_TYPE_UNKNOWN) {
                    return gameMenu.getMenuSetting().getGestureMode();
                } else if (gameMenu.getHitResultType() == H2CO3LauncherBridge.HIT_RESULT_TYPE_BLOCK) {
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if (gameMenu.getCursorMode() == H2CO3LauncherBridge.CursorEnabled) {
            if (gameMenu.getMenuSetting().getMouseMoveMode() == MouseMoveMode.CLICK) {
                ...
                switch (event.getActionMasked()) {
                    ...
                    case MotionEvent.ACTION_UP:
                        Choreographer.getInstance().postFrameCallbackDelayed(frameTimeNanos -> {
                            gameMenu.getInput().sendKeyEvent(H2CO3LauncherInput.MOUSE_LEFT, false);
                        }, 33);
                        break;
                    default:
                        break;
            ...
            switch (event.getActionMasked()) {
                ...
                case MotionEvent.ACTION_UP:
                    ...
                    if (cancelMouseRight) {
                        gameMenu.getInput().sendKeyEvent(H2CO3LauncherInput.MOUSE_RIGHT, false);
                    }
                    cancelMouseLeft = false;
                    cancelMouseRight = false;
                    if (System.currentTimeMillis() - downTime <= 100
