...
import org.koishi.launcher.h2co3core.fakefx.beans.property.SimpleDoubleProperty
...
class VersionSetting : Cloneable {
    ...
    var isUsesGlobal: Boolean
        /**
         * H2CO3Launcher Version Settings have been divided into 2 parts.
         * 1. Global settings.
         * 2. Version settings.
    ...
    val maxMemoryProperty: IntegerProperty =
        SimpleIntegerProperty(this, "maxMemory", MemoryUtils.findBestRAMAllocation(
            H2CO3LauncherTools.CONTEXT))
    var maxMemory: Int
        /**
    ...
    var isNotCheckJVM: Boolean
        /**
         * True if H2CO3Launcher does not check JVM validity.
         */
    ...
    var isNotCheckGame: Boolean
        /**
         * True if H2CO3Launcher does not check game's completeness.
         */
    ...
    val rendererProperty: ObjectProperty<H2CO3LauncherConfig.Renderer> =
        SimpleObjectProperty(this, "render", H2CO3LauncherConfig.Renderer.RENDERER_GL4ES)
    var renderer: H2CO3LauncherConfig.Renderer
    ...
    class Serializer : JsonSerializer<VersionSetting?>, JsonDeserializer<VersionSetting?> {
        override fun serialize(
            ...
            return JsonObject().apply {
                ...
                addProperty(
                    "maxMemory",
                    if (src.maxMemory <= 0) MemoryUtils.findBestRAMAllocation(H2CO3LauncherTools.CONTEXT) else src.maxMemory
                )
        ...
        override fun deserialize(
            ...
            var maxMemoryN = parseJsonPrimitive(
                ...
                MemoryUtils.findBestRAMAllocation(H2CO3LauncherTools.CONTEXT)
            )
            if (maxMemoryN <= 0) maxMemoryN = MemoryUtils.findBestRAMAllocation(H2CO3LauncherTools.CONTEXT)
            return VersionSetting().also { vs ->
                ...
                if (!RendererPlugin.isAvailable() && vs.customRenderer != "") {
                    vs.renderer = H2CO3LauncherConfig.Renderer.entries.toTypedArray()[0]
                    vs.customRenderer = ""
                }
            }
        }
