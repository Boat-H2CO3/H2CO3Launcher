...
import org.koishi.launcher.h2co3.control.EditViewDialog;
import org.koishi.launcher.h2co3.control.GameMenu;
import org.koishi.launcher.h2co3.control.GestureMode;
import org.koishi.launcher.h2co3.control.MouseMoveMode;
...
public class ControlButton extends AppCompatButton implements CustomView {
    ...
    public ControlButton(@NonNull Context context, GameMenu gameMenu, ViewListener listener) {
        ...
        screenWidth = AndroidUtils.getScreenWidth(H2CO3LauncherApplication.getCurrentActivity());
        screenHeight = AndroidUtils.getScreenHeight(H2CO3LauncherApplication.getCurrentActivity());
    ...
    private void refreshBaseInfo(ControlButtonData data) {
        ...
        if (menu.isEditMode()) {
            ...
            visibilityProperty().bind(Bindings.createBooleanBinding(() -> isParentVisibility() && (data.getBaseInfo().getVisibilityType() == BaseInfoData.VisibilityType.ALWAYS ||
                            (data.getBaseInfo().getVisibilityType() == BaseInfoData.VisibilityType.IN_GAME && menu.getCursorMode() == H2CO3LauncherBridge.CursorDisabled) ||
                            (data.getBaseInfo().getVisibilityType() == BaseInfoData.VisibilityType.MENU && menu.getCursorMode() == H2CO3LauncherBridge.CursorEnabled)),
        ...
        if (menu.isEditMode()) {
            ...
            switch (event.getActionMasked()) {
                case MotionEvent.ACTION_DOWN:
                    ...
                    downX = event.getX();
                    downY = event.getY();
                    initialX = menu.getCursorMode() == H2CO3LauncherBridge.CursorEnabled ? menu.getCursorX() : menu.getPointerX();
                    initialY = menu.getCursorMode() == H2CO3LauncherBridge.CursorEnabled ? menu.getCursorY() : menu.getPointerY();
                    positionX = getX();
                    positionY = getY();
                    downTime = System.currentTimeMillis();
