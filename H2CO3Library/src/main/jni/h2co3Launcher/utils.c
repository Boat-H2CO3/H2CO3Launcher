...
void printVulkanInfo(void *handle) {
    ...
    if (vkCreateInstance(&instanceCreateInfo, 0, &instance) != 0) {
        H2CO3LAUNCHER_INTERNAL_LOG("vkCreateInstance error");
        return;
    }
    uint32_t gpu_count;
    if (vkEnumeratePhysicalDevices(instance, &gpu_count, NULL) != 0) {
        H2CO3LAUNCHER_INTERNAL_LOG("vkEnumeratePhysicalDevices error");
        return;
    }
    ...
    H2CO3LAUNCHER_INTERNAL_LOG("Vulkan api version:%d.%d.%d",
