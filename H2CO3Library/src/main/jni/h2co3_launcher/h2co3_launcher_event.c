...
JNIEXPORT void JNICALL Java_org_koishi_launcher_h2co3launcher_bridge_H2CO3LauncherBridge_refreshHitResultType(JNIEnv *env, jobject thiz) {
    if (injectorCallback)
        injectorCallback();
}
...
JNIEXPORT void JNICALL Java_org_koishi_launcher_h2co3launcher_bridge_H2CO3LauncherBridge_pushEvent(JNIEnv* env, jclass clazz, jlong time, jint type, jint p1, jint p2) {
    if (!h2co3Launcher->has_event_pipe) {
        return;
    }
    if (pthread_mutex_lock(&h2co3Launcher->event_queue_mutex)) {
...
JNIEXPORT void JNICALL Java_org_koishi_launcher_h2co3launcher_bridge_H2CO3LauncherBridge_setEventPipe(JNIEnv* env, jclass clazz) {
