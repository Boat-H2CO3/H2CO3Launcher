...
static jlong ndlopen_bugfix(__attribute__((unused)) JNIEnv *env,
    ...
    if(strstr(filename, "libvulkan.so") == filename) {
        H2CO3LAUNCHER_INTERNAL_LOG("LWJGL linkerhook: replacing load for libvulkan.so with custom driver");
...
void installLwjglDlopenHook(JavaVM* vm) {
    H2CO3LAUNCHER_INTERNAL_LOG("LwjglLinkerHook:%s", "Installing LWJGL dlopen() hook");
    JNIEnv *env = NULL;
    (*vm)->GetEnv(vm, (void **) &env, JNI_VERSION_1_4);
    jclass dynamicLinkLoader = (*env)->FindClass(env, "org/lwjgl/system/linux/DynamicLinkLoader");
    if(dynamicLinkLoader == NULL) {
        H2CO3LAUNCHER_INTERNAL_LOG( "LwjglLinkerHook:%s", "Failed to find the target class");
    ...
    if((*env)->RegisterNatives(env, dynamicLinkLoader, ndlopenMethod, 1) != 0) {
        H2CO3LAUNCHER_INTERNAL_LOG( "LwjglLinkerHook:%s", "Failed to register the hooked method");
        (*env)->ExceptionClear(env);
    }
}