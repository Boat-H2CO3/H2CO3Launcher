...
JNIEXPORT jstring JNICALL Java_org_lwjgl_glfw_CallbackBridge_nativeClipboard(JNIEnv* env, __attribute__((unused)) jclass clazz, jint action, jbyteArray copySrc) {
#ifdef DEBUG
    H2CO3Launcher_INTERNAL_LOG("Debug: Clipboard access is going on\n", pojav_environ->isUseStackQueueCall);
#endif
    ...
    H2CO3Launcher_INTERNAL_LOG("Clipboard: Converting string\n");
    char *copySrcC;
    ...
    H2CO3Launcher_INTERNAL_LOG("Clipboard: Calling 2nd\n");
...
#ifdef DEBUG
    H2CO3Launcher_INTERNAL_LOG("Debug: Changing input state, isReady=%d, pojav_environ->isUseStackQueueCall=%d\n", inputReady, pojav_environ->isUseStackQueueCall);
#endif
...
#ifdef DEBUG
    H2CO3Launcher_INTERNAL_LOG("Sending cursor position \n");
#endif
    if (pojav_environ->GLFW_invoke_CursorPos && pojav_environ->isInputReady) {
#ifdef DEBUG
        H2CO3Launcher_INTERNAL_LOG("pojav_environ->GLFW_invoke_CursorPos && pojav_environ->isInputReady \n");
#endif
...
static bool tryCriticalNative(JNIEnv *env) {
    ...
    jclass criticalNativeTest = (*env)->FindClass(env, "org/koishi/launcher/h2co3/core/launch/CriticalNativeTest");
